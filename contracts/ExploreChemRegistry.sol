...
contract ExploreChemProofRegistry is IReceiver, IERC721, IERC5192 {
    ...
    /// @notice Latest named hashes and statuses for direct frontend consumption.
    /// @dev All statuses are initialized to PENDING when the evidence is submitted.
    struct CurrentProofState {
        bytes32 mufHash;
        bytes32 toleranceHash;
        bytes32 elementalHash;
        bytes32 yieldHash;
        bytes32 multiStreamHash;
        bytes32 auditHash;
        CheckStatus mufStatus;
        CheckStatus toleranceStatus;
        CheckStatus elementalStatus;
        CheckStatus yieldStatus;
        CheckStatus multiStreamStatus;
    ...
    mapping(ProofType => bytes32) public expectedWorkflowId;
    ...
    mapping(bytes32 => mapping(ProofType => bytes32)) public latestProofId;
    ...
    function setExpectedWorkflowId(ProofType proofType, bytes32 workflowId) external onlyOwner {
    ...
    function submitEvidence(
        ...
        state.toleranceStatus = CheckStatus.PENDING;
        state.elementalStatus = CheckStatus.PENDING;
        state.yieldStatus = CheckStatus.PENDING;
        state.multiStreamStatus = CheckStatus.PENDING;
        state.auditStatus = CheckStatus.PENDING;
    ...
    function onReport(bytes calldata metadata, bytes calldata report) external override {
        ...
        ProofType proofType = _decodeProofType(decoded.proofType);
    ...
    function _updateCurrentProofState(
        ...
        if (proofType == ProofType.MUF) {
            state.mufHash = committedHash;
            state.mufStatus = status;
        } else if (proofType == ProofType.TOLERANCE) {
            state.toleranceHash = committedHash;
            state.toleranceStatus = status;
        } else if (proofType == ProofType.ELEMENTAL) {
            state.elementalHash = committedHash;
            state.elementalStatus = status;
        } else if (proofType == ProofType.YIELD) {
            state.yieldHash = committedHash;
            state.yieldStatus = status;
        } else if (proofType == ProofType.MULTI_STREAM) {
            state.multiStreamHash = committedHash;
            state.multiStreamStatus = status;
        } else {
            state.auditHash = committedHash;
            state.auditStatus = status;
        }
    }

    function _decodeProofType(uint8 value) internal pure returns (ProofType proofType) {
        if (value < uint8(ProofType.MUF) || value > uint8(ProofType.AUDIT)) {
            revert InvalidProofType(value);
        }
        return ProofType(value);
    }

    function _requireProofType(ProofType proofType) internal pure {
